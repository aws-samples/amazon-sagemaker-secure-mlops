...
def lambda_handler(event, context):
    try:
        ...
        sm_domain_id = get_file(job_data["inputArtifacts"][0], user_param["FileName"]).get("SageMakerDomainId")

        vpc_id = delete_efs(sm_domain_id)
        
        if vpc_id and user_param.get("VPC") == "delete":
